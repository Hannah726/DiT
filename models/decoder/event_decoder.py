...
class EventDecoder(nn.Module):
    ...
    def forward(self, event_latents, return_logits=False):
        ...
        Returns:
            If return_logits=True:
                dict with keys 'token', 'type', 'dpe', each (B, N, L, vocab_size)
            Else:
                dict with keys 'token', 'type', 'dpe', each (B, N, L)
        """
    ...
    def compute_reconstruction_loss(self, event_latents, target_tokens, target_types, target_dpes, mask=None):
        ...
        if mask is not None:
            ...
            token_loss = (token_loss * mask_flat).sum() / mask_flat.sum()
            type_loss = (type_loss * mask_flat).sum() / mask_flat.sum()
            dpe_loss = (dpe_loss * mask_flat).sum() / mask_flat.sum()
        else:
            token_loss = token_loss.mean()
            type_loss = type_loss.mean()
            dpe_loss = dpe_loss.mean()
